...
test.beforeAll( async({browser})=>
{
    ...
    expect(page).toHaveURL("https://rozetka.com.ua/")
})

test('Rozetka Playwrihgt test', async ({browser})=>{
    ...
    await page.locator("[name='search']").type("iPhone 14")
    
    await page.keyboard.press('Enter')
    //await page.pause();
    await new Promise(resolve => setTimeout(resolve, 2000))
  
    await expect(page.locator("[name='search']")).toBeEmpty()
    
    //const searchResult = await page.locator('.goods-tile__inner');
    const searchResult = await page.locator('.goods-tile__heading');
    ...
    //await page.pause();
    const h1Header = await page.locator('.product__title')
